...
namespace utils::http
{
	...
	int get_data_stream(const std::string& url, const headers& headers = {}, const std::function<void(size_t)>& _progress_callback = {},
		const std::function<void(const char*, size_t)>& _stream_callback = {}, uint32_t retries = 2);
