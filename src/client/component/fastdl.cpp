...
namespace fastdl
{
	namespace
	{
		...
		void show_ingame_error(const std::string& error)
		{
			...
			{
				game::UI_OpenErrorPopupWithMessage(0, 0x100, error.data());
			}, scheduler::main);
		}
		...
		std::pair<std::vector<updater::file_info>, updater::file_info> get_manifest_files(const std::string& base_url, const std::string& mapname)
		{
			...
			// Separate workshop.json from other files
		...
		void perform_download(const download_context& context)
		{
			try
			{
				...
				// Check which files need updating (excluding workshop.json)
				...
				ui_scripting::show_message_dialog("Map downloaded", "Finished downloading map files. Joining server...");
				ui.done_update();
	...
	void fastdl_ui::update_files(const std::vector<updater::file_info>& files)
	{
		...
		this->progress_ui_.set_title("BOIII FastDL");
	...
	public:
		...
		{
			// Component hooks will be added when integrating with workshop
		}
